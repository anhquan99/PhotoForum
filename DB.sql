...
BEGIN TRANSACTION
	...
	ALTER TABLE POST ADD STATUS VARCHAR(50)
	ALTER TABLE FORUM_USER ADD ROLE VARCHAR(50)
	--LINK
	...
	END
	GO
	CREATE PROCEDURE FIND_IMG_WITH_TAG @TAG_NAME nvarchar(250)
	AS
	BEGIN
		SELECT * FROM TAG_IMG WHERE TAG_NAME = @TAG_NAME ORDER BY IMG_ID DESC
	END
 	alter table TAG_IMG DROP CONSTRAINT FK_TAG_TAG
  	ALTER TABLE TAG_IMG DROP CONSTRAINT FK_TAG_IMG 
  	ALTER TABLE TAG_IMG ADD CONSTRAINT FK_TAG_TAG FOREIGN KEY (TAG_NAME) REFERENCES TAG(TAG_NAME) ON DELETE CASCADE
  	ALTER TABLE TAG_IMG ADD CONSTRAINT FK_TAG_IMG FOREIGN KEY (IMG_ID) REFERENCES IMG(IMG_ID) ON DELETE CASCADE
	go
	CREATE PROCEDURE FIND_IMG_WITH_TAG_AND_USERNAME @TAG_NAME varchar(250), @USERNAME VARCHAR(250)
	AS
	BEGIN
		SELECT IMG_ID FROM TAG_IMG WHERE TAG_NAME = @TAG_NAME INTERSECT SELECT IMG_ID FROM IMG WHERE USERNAME = @USERNAME
	END
COMMIT;
